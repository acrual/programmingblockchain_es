...
class ElementoCampo:
    ...
    def __add__(self, otro):
        if self.primo != otro.primo:
            raise TypeError('No se pueden sumar dos números en campos distintos')
    ...
    def __pow__(self, n):
        ...
        # self.num**(p-1) % p == 1
        # puede que quieras usar el operador % sobre n
        primo = self.primo
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        ...
        # los que no estén no deberían dar un ValueError
        primo = 223
    ...
    def prueba_sumar(self):
        # comprueba que las siguientes sumas en la curva y^2=x^3-7 sobre F_223:
        # (192,105) + (17,56)
        # (47,71) + (117,141)
        # (143,98) + (76,66)
        primo = 223
    ...
    def prueba_rmul(self):
        ...
        for s, x1_bruto, y1_bruto, x2_bruto, y2_bruto in multiplicaciones:
            ...
            if x2_bruto is None:
                ...
                y2 = ElementoCampo(y2_bruto, primo)
                p2 = Punto(x2, y2, a, b)
            # compruebaque el producto sea igual al punto esperado
...
class PuntoS256(Punto):
    ...
    def __rmul__(self, coeficiente):
        # queremos hacer el módulo por N para hacer esto más simple
        coef = coeficiente % N
    ...
    def parsear(self, sec_bin):
        '''devuelve un objeto Punto desde un sec binario comprimido (not hex)
        '''
        if sec_bin[0] == 4:
...
class ClavePrivada:
    ...
    def firmar(self, z):
        # necesitamos un número k aleatorio: randint(0, 2**256)
        k = randint(0, 2**256)
        # r es la coordenada x del punt k*G resultante
        r = (k * G).x.num
