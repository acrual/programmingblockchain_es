...
class Script:
    ...
    def parsear(cls, s):
        ...
        # inicializar el array de cosas
        cosas = []
        # inicializa el número de bytes que hemos leído a 0
        contador = 0
        # haz un bucle hasta que leamos "longitud" bytes
        while contador < longitud:
            ...
            # si el byte actual está entre 1 y 75 incluidos
            if byte_actual >= 1 and byte_actual <= 75:
                # tenemos una cosa establecida como n para que sea el byte actual
                n = byte_actual
    ...
    def serializar(self):
        ...
        for cosa in self.cosas:
            ...
            if type(cosa) == int:
                ...
                # de otra forma, esto es un elemento
                # obté la longitud en bytes
                longitud = len(cosa)
    ...
    def firma(self):
        '''devuelve el elemento firma asumiendo un script sig p2pkh'''
        return self.cosas[0]

    def sec_pubkey(self):
        '''devuelve el elemento pubkey asumiendo un script sig p2pkh'''
        return self.cosas[1]

    def dire(self, testnet=False):
        '''Devuelve la dirección correspondiente al script'''
        # HACK: cuenta cuántos elementos para determinar de qué tipo es
        if len(self.cosas) == 5:  # p2pkh
            # hash160 es el tercer elemento
            h160 = self.cosas[2]
            # convierte a dirección p2pkh usando h160_a_dire_p2pkh (recuerda testnet)
            return h160_a_dire_p2pkh(h160, testnet)
        elif len(self.cosas) == 3:  # p2sh
            # hash160 es el 2º elemento
            h160 = self.cosas[1]
            # convierte a dirección p2sh usando h160_a_dire_p2sh (recuerda testnet)
