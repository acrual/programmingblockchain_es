...
class Bloque:
    ...
    def __init__(self, versión, bloque_previo, raiz_merkle, timestamp, bits, nonce, tx_hashes=None):
    ...
    def parsear(cls, s):
        ...
        # raiz_merkle - 32 bytes, little endian (use [::-1] to reverse)
        raiz_merkle = s.read(32)[::-1]
        ...
        nonce = s.read(4)
        # initialize class
        return cls(versión, bloque_previo, raiz_merkle, timestamp, bits, nonce)
    ...
    def hash(self):
        '''Devuelve el doble_sha256 interpretado como little endian del bloque'''
        # serializar
        s = self.serializar()
        # doble_sha256
        sha = doble_sha256(s)
    ...
    def bip9(self):
        ...
        # BIP9 está señalizado si los top 3 bits son 001
        # recuerda que la versión es 32 bytes así que mueve a la derecha 29 (>> 29) y mira si
        # eso es 001
        return self.versión >> 29 == 0b001

    def bip91(self):
        '''Devuelve si este bloque señaliza estar listo para BIP91'''
        # BIP91 se señaliza si el 5º bit desde la derecha es 1
    ...
    def bip141(self):
        ...
        # BIP141 se señaliza si el 2º bit desde la derecha es 1
        # mueve 1 bit a la derecha y mira si el último bit es 1
        return self.versión >> 1 & 1 == 1

    def objetivo(self):
        '''Devuelve el objetivo de la prueba-de-trabajo basado en los bits'''
    ...
    def dificultad(self):
        ...
        # la mínima dificultad tiene bits que son iguales a 0xffff001d
        mínima = 0xffff * 256**(0x1d - 3)
        return mínima / self.objetivo()
    ...
    def comprobar_pow(self):
        ...
        return prueba < self.objetivo()

    def validar_raiz_merkle(self):
        '''Obtiene la raiz_merkle de los tx_hashes y comprueba que es la misma que
        la raiz merkle de este bloque.
        '''
        # dale la vuelta a todos los hashes de las transacciones (self.tx_hashes)
        # obtén la raíz Merkle
        # dale la vuelta a la raiz Merkle
        # devuelve si la raíz self.merkle root es la misma que
        # el reverso de la raiz merkle calculada
...
class PruebaBloque(TestCase):
    ...
    def prueba_objetivo(self):
        ...
        stream = BytesIO(bloque_bruto)
        bloque = Bloque.parsear(stream)
        self.assertEqual(bloque.objetivo(), 0x13ce9000000000000000000000000000000000000000000)
    ...
    def prueba_validar_raiz_merkle(self):
        hashes_hex = [
        ...
        self.assertTrue(bloque.validar_raiz_merkle())
