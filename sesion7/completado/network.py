...
class NetworkEnvelope:
    ...
    def parsear(cls, s, testnet=False):
        ...
        # comando 12 bytes
        comando = s.read(12)
        # quítale los 0's que arrastra
        comando = comando.strip(b'\x00')
        # longitud de carga 4 bytes, little endian
        longitud_carga = little_endian_a_int(s.read(4))
        # checksum 4 bytes, primeros cuatro de double-sha256 de carga
        checksum = s.read(4)
        # carga de de longitud longitud_carga
        carga = s.read(longitud_carga)
        # verificar checksum
        checksum_calculado = doble_sha256(carga)[:4]
        if checksum_calculado != checksum:
            raise RuntimeError('checksums no son iguales')
    ...
    def serializar(self):
        ...
        # checksum 4 bytes, los primeros cuatro de double-sha256 de carga
...
class MensajeVersión:
    ...
    def serializar(self):
        '''Serializa este mensaje para ser enviado por la red'''
        ...
        # servicios receptor es 8 bytes little endian
        ...
        # puerto receptor es 2 bytes, little endian debería ser 0
        res += int_a_little_endian(self.puerto_receptor, 2)
        # sender servicios es 8 bytes little endian
        res += int_a_little_endian(self.servicios_emisor, 8)
        # IPV4 es 10 00 bytes y 2 ff bytes luego ip emisor
        res += b'\x00' * 10 + b'\xff\xff' + self.ip_emisor
        # puerto emisor es 2 bytes, little endian debería ser 0
        res += int_a_little_endian(self.puerto_emisor, 2)
        # nonce debería ser 8 bytes
        res += self.nonce
        ...
        # el último bloque es 4 bytes little endian
...
class ObtenerMensajeCabeceras:
    ...
    def serializar(self):
        '''Serializar este mensaje para enviarlo a la red'''
        ...
        # el bloque final también es little-endian
...
class MensajeCabeceras:
    ...
    def parsear(cls, stream):
        ...
        bloques = []
        # haz un bucle tantas veces como número de cabeceras haya
        for _ in range(num_headers):
            ...
            # num_txs debería ser 0 o elevar un RuntimeError
            if num_txs != 0:
...
class PruebaMensajeCabeceras(TestCase):
    ...
    def prueba_parsear(self):
        ...
        headers = MensajeCabeceras.parsear(stream)
        self.assertEqual(len(headers.bloques), 2)
        for b in headers.bloques:
...
class ObtenerMensajeDatos:
    ...
    def add_data(self, tipo_dato, identificador):
        self.data.append((tipo_dato, identificador))

    def serializar(self):
        # comienza con el número de elementos como un varint
        res = codificar_varint(len(self.data))
        for tipo_dato, identificador in self.data:
            # el tipo de dato es 4 bytes little endian
            res += int_a_little_endian(tipo_dato, 4)
            # identificador nnecesita estar en little endian
            res += identificador[::-1]
        return res
...
class PruebaObtenerMensajeDatos(TestCase):
    ...
    def prueba_serializar(self):
        ...
        obtener_datos.add_data(TIPO_DATOS_BLOQUE_FILTRADOS, bloque1)
        bloque2 = bytes.fromhex('00000000000000beb88910c46f6b442312361c6693a7fb52065b583979844910')
        obtener_datos.add_data(TIPO_DATOS_BLOQUE_FILTRADOS, bloque2)
...
class NodoSimple:
    ...
    def handshake(self):
        ...
        # espera por un mensaje de verack
    ...
    def send(self, comando, carga):
        ...
        # envía el envelope serializado a través del socket usando sendall
    ...
    def esperar_comandos(self, comandos):
        '''Espera a uno de los comandos de la lista'''
        # inicializa el comando que tengamos, que debería ser None
        comando = None
        # haz un bucle hasta que el comando esté en los comandos que queramos
        ...
        # devuelve el último envelope que tengamos
        return envelope
