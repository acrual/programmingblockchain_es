...
class ÁrbolMerkle:
    ...
    def __init__(self, total):
        ...
        # inicializa el atributo de los nodos para mantener el árbol actual
        self.nodes = []
        # haz un bucle sobre el número de niveles (profundidad_máxima+1)
        for profundidad in range(self.profundidad_máxima + 1):
            # el número de  number of cosas a esta profundidad es
            ...
            # añade los hashes de este nivel al árbol merkle
            self.nodes.append(nivel_hashes)
        # apunta a la raíz (profundidad=0, index=0)
    ...
    def __repr__(self):
        ...
        for profundidad, nivel in enumerate(self.nodes):
            for index, h in enumerate(nivel):
                short = '{}...'.format(h.hex()[:8])
                if profundidad == self.profundidad_actual and index == self.índice_actual:
                    res += '*{}*, '.format(short[:-2])
                else:
                    res += '{}, '.format(short)
            res += '\n'
    ...
    def subir(self):
        # reduce profundidad en 1 y divide en dos el índice
        self.profundidad_actual -= 1
        self.índice_actual //= 2

    def izquierda(self):
        # aumenta profundidad en 1 y dobla el índice
        self.profundidad_actual += 1
        self.índice_actual *= 2
    ...
    def establecer_nodo_actual(self, value):
    ...
    def poblar_árbol(self, flag_bits, hashes):
        ...
        while self.raíz() is None:
            # si tenemos una hoja, sabemos la posición de este hash
            if self.es_hoja():
                # obtén el próximo bit desde flag_bits: flag_bits.pop(0)
                flag_bits.pop(0)
                # establece el nodo actual en el árbol merkle al siguiente hash: hashes.pop(0)
            ...
            # else
                ...
                if hash_izquierdo is None:
                    # si el próximo flag bit es 0, el siguiente hash es nuestro nodo actual
                    if flag_bits.pop(0) == 0:
                        # establece el nodo actual para ser el próximo hash
                        self.establecer_nodo_actual(hashes.pop(0))
                        # el sub-árbol no necesita cálculo, sube arriba
                        self.subir()
                    # else
                    else:
                ...
                # Ejercicio 6.2: else
                    ...
                    # hemos completado este sub-árbol, sube arriba
                # Ejercicio 7.2: si el hash derecho existe
                    ...
                    # else
                        ...
                        # hemos completado este sub-árbol, sube un nivel
                        self.subir()
                # else
                else:
                    ...
                    self.subir()
        if len(hashes) != 0:
            raise RuntimeError('no todos los hashes se han consumido {}'.format(len(hashes)))
        for flag_bit in flag_bits:
            if flag_bit != 0:
                raise RuntimeError('no todos los flag bits se han consumido')
