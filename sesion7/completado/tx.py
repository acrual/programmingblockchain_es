...
class Tx:
    ...
    def parsear(cls, s, testnet=False):
        ...
        # versión tiene 4 bytes, little-endian, interpretalo como entero
        versión = little_endian_a_int(s.read(4))
        # num_inputs es un varint, usa read_varint(s)
        num_inputs = read_varint(s)
        # cada input necesita parsea
        inputs = []
    ...
    def comisión(self):
        ...
        # inicializa el input sum y output sum
    ...
    def sig_hash(self, índice_input, tipo_hash):
        ...
        ser firmado para el índice índice_input'''
        ...
        # el script_sig del input_firmante debería ser el script_pubkey
    ...
    def verificar_input(self, índice_input):
        ...
        # parsear el punto desde el formato sec (tx_in.sec_pubkey())
        punto = PuntoS256.parsear(tx_in.sec_pubkey())
        # parsear la firma desde el formato der (tx_in.firma_der())
    ...
    def es_coinbase(self):
        ...
        # comprueba que haya exactamente 1 input
        if len(self.tx_ins) != 1:
            return False
        # obtén el primer input
        primer_input = self.tx_ins[0]
        # comprueba que el primer input de tx_previa sea b'\x00' * 32 bytes
        if primer_input.tx_previa != b'\x00' * 32:
            return False
        # comprueba que el primer input de índice_previo sea 0xffffffff
...
class TxIn:
    ...
    def valor(self, testnet=False):
        '''Consigue el valor del outpoint mirando el hash de la tx
    ...
    def script_pubkey(self, testnet=False):
        '''Consigue el scriptPubKey mirando el hash de la tx
        Devuelve un objeto Script
        '''
        # usa self.obtener_tx para conseguir la transacción
        tx = self.obtener_tx(testnet=testnet)
        # obtén el output en self.índice_previo
    ...
    def sec_pubkey(self):
        '''devuelve el formato SEC format público si el script_sig tiene uno'''
