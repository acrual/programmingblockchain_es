...
def codificar_base58(s):
    # determina con cuantos 0 bytes (b'\x00') comienza s
    contador = 0
    for c in s:
        if c == 0:
            contador += 1
...
def decodificar_base58(s):
    ...
    if doble_sha256(combinado[:-4])[:4] != checksum:
        raise RuntimeError('bad address: {} {}'.format(checksum, doble_sha256(combinado)[:4]))
...
def codificar_varint(i):
    '''condifica un entero como varint'''
    if i < 0xfd:
        return bytes([i])
    elif i < 0x10000:
...
def h160_a_dire_p2sh(h160, testnet=False):
    '''Toma una secuencia de bytes y devuelve una cadena de dirección p2sh'''
...
def raíz_merkle(hashes):
    ...
    # devuelve la primera cosa de nivel_actual
...
class PruebaAyudante(TestCase):
    ...
    def prueba_base58(self):
        ...
        h160 = decodificar_base58(addr).hex()
