...
class Script:
    ...
    @classmethod
    def parsear(cls, s):
        # obtén la longitud del campo entero
        longitud = read_varint(s)
        # inicializa cosas así
        cosas = []
        # inicializa el número de bytes que debemos leer a 0
        contador = 0
        # haz un bucle hasta que hayamos leído la longitud en bytes
        while contador < longitud:
            # obtén el byte actual
            actual = s.read(1)
            # incrementa los bytes que hemos leído
            contador += 1
            # convierte el byte actual en un entero
            byte_actual = actual[0]
            # si el byte actual está entre 1 y 75 ambos incluidos
            if byte_actual >= 1 and byte_actual <= 75:
                # tenemos una cosa n igual al byte actual
                n = byte_actual
                ...
                # tenemos un op code. Establece en byte actual a op_code
                op_code = byte_actual
    ...
    def serializar(self):
        ...
        for cosa in self.cosas:
            ...
            if type(cosa) == int:
                ...
                # añade el resultado tanto la longitud como la cosa
                res += prefijo + cosa
    ...
    def sec_pubkey(self):
        '''devuelve el elemento pubkye asumiendo script sig p2pkh'''
        return self.cosas[1]

    def dire(self, testnet=False):
        '''Devuelve la dirección correspondiente al script'''
        # HACK: simplemente cuenta cuántos elementos para determinar de qué tipo es
        if len(self.cosas) == 5:  # p2pkh
            # hash160 es el tercer element
