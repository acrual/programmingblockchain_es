...
class Tx:
    ...
    def comisión(self):
        ...
        # itera los outputs
        for tx_out in self.tx_outs:
            # para cada output obtén la cantidad y añade output sum
    ...
    def verificar_input(self, indice_input):
        ...
        # obtén el tipo_hash desde el input (tx_in.tipo_hash())
    ...
    def es_coinbase(self):
        ...
        # comprueba la tx_previa del primer input es b'\x00' * 32 bytes
        if primer_input.tx_previa != b'\x00' * 32:
            return False
        # comprueba que el índice previo del primer input es 0xffffffff
        if primer_input.indice_previo != 0xffffffff:
            return False
        return True
...
class TxIn:
    ...
    @classmethod
    def parsear(cls, s):
        '''Toma el stream de bytes y parsea el tx_input al comienzo
        ...
        # sequence es 4 bytes, little-endian, interpreta como int
    ...
    def sec_pubkey(self):
        '''devuelve el formato público SEC si el script_sig tiene uno'''
...
class TxOut:
    ...
    def serializar(self):
        '''Devuelve la serialización de bytes del output de la transacción'''
