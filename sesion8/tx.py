...
class Tx:
    ...
    def comisión(self):
        ...
        for tx_out in self.tx_outs:
            # para cada output obtén la cantidad y añádela al output sum
            output_sum += tx_out.cantidad
        # devuelve input sum - output sum
        return input_sum - output_sum

    def sig_hash(self, índice_input, tipo_hash):
        '''Devuelve la representación entera del hash que necesita
        ser verificar_inputdo para índice índice_input'''
        # crea un nuevo set de tx_ins (alt_tx_ins)
        alt_tx_ins = []
        # itera por self.tx_ins
        for tx_in in self.tx_ins:
            # crea un nuevo TxIn quetenga un script_sig en blanco (b'') y añádelo a alt_tx_ins
        ...
        # obtén el input en el índice_input
        input_verificar_inputnte = alt_tx_ins[índice_input]
        # obtén el script_pubkey del input
        script_pubkey = input_verificar_inputnte.script_pubkey(self.testnet)
        # el script_sig del input_verificar_inputnte debería ser script_pubkey
        input_verificar_inputnte.script_sig = script_pubkey
        # crea una transacción alternativa con el tx_ins modificado
    ...
    def es_coinbase(self):
        ...
        if primer_input.tx_previa != b'\x00' * 32:
            return False
        # comprueba que el primer input de índice_previo es 0xffffffff
        if primer_input.índice_previo != 0xffffffff:
            return False
        return True

    def altura_coinbase(self):
        '''Devuelve la altura del bloque en el que está esta transacción coinbase
        Devuelve None si esta transacción no es coinbase
        '''
        # si NO es coinbase, devuelve None
...
class TxIn:
    ...
    @classmethod
    def parsear(cls, s):
        '''Toma un stream de bytes y parsea la tx_input al comienzo
        ...
        # puedes usar Script.parsear para obtener el auténtico script
        script_sig = Script.parsear(s)
        # sequence es 4 bytes, little-endian, interpretalo comos int
    ...
    @classmethod
    def crea_cache(cls, tx_id, bruto):
    ...
    def obtener_tx(self, testnet=False):
        if self.tx_previa not in self.cache:
            ...
            if bruto[4] == 0:
                 # esto es segwit, así que conviértelo en no-segwit
    ...
    def valor(self, testnet=False):
        '''Obtén el valor del outpoint mirando el hash de la tx
        Devuelve la cantidad en satoshis
        '''
        # usa self.obtener_tx para obtener la tx
    ...
    def sec_pubkey(self):
        '''devuelve el format público SEC si el script_sig has one'''
