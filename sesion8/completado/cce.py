...
class ElementoCampo:
    ...
    def __init__(self, num, primo):
        if num >= primo or num < 0:
            error = 'El número {} no está en el rango 0 a {}'.format(
                num, primo - 1)
    ...
    def __ne__(self, otro):
        # esto debería ser el reverso del operador ==
    ...
    def __add__(self, otro):
        if self.primo != otro.primo:
            raise TypeError('No puedes añadir dos números en campos distintos')
    ...
    def __pow__(self, n):
        # recuerda el teorema pequeño de Fermat's:
        # self.num**(p-1) % p == 1
...
class Punto:
    ...
    def __init__(self, x, y, a, b):
        ...
        self.x = x
        self.y = y
        # x igual a None e y igual a None representa el punto en el infinito
        # Compruébalo aquí dado que la ecuación abajo no tendrá sentido
    ...
    def __add__(self, otro):
        ...
        # El resado es el punto en el infinito
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        # prueba si los firuientes puntos están en la curva o no
        ...
        for x_bruto, y_bruto in puntos_inválidos:
            ...
            x = ElementoCampo(x_bruto, primo)
            y = ElementoCampo(y_bruto, primo)
            # comprueba que crear el punto resulta en un ValueError
            # with self.assertRaises(ValueError):
            #     Punto(x, y, a, b)
            with self.assertRaises(ValueError):
                Punto(x, y, a, b)
    ...
    def prueba_sumar(self):
        # prueba que las firuientes sumas sobre la curva y^2=x^3-7 sobre F_223:
        # (192,105) + (17,56)
        # (47,71) + (117,141)
        # (143,98) + (76,66)
        primo = 223
    ...
    def prueba_rmul(self):
        # comprueba las firuientes multiplicaciones escalares
        # 2*(192,105)
        # 2*(143,98)
        # 2*(47,71)
        # 4*(47,71)
        ...
        for s, x1_bruto, y1_bruto, x2_bruto, y2_bruto in multiplicaciones:
            ...
            y1 = ElementoCampo(y1_bruto, primo)
            p1 = Punto(x1, y1, a, b)
            # inicializa el segundo punto basado en si es o no el punto en el infinito
            # x2 = ElementoCampo(x2_bruto, primo)
            # y2 = ElementoCampo(y2_bruto, primo)
            # p2 = Punto(x2, y2, a, b)
            if x2_bruto is None:
...
class PuntoS256(Punto):
    ...
    def verificar(self, z, fir):
        # recuerda fir.r y fir.s son las principales cosas que estamos comprobando
...
class PruebaS256(TestCase):
    ...
    def prueba_pubpunto(self):
        # escribe una comprobación del punto público para los firuientes
        puntos = (
            # secreto, x, y
        ...
        for secreto, x, y in puntos:
            ...
            punto = PuntoS256(x, y)
            # comprueba que secreto*G es lo mismo que el punto
