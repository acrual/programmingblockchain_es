...
class Script:
    ...
    def parsear(cls, s):
        ...
        contador = 0
        # haz un bucle hasta que hayamos leido longitud bytes
        while contador < longitud:
            ...
            if byte_actual >= 1 and byte_actual <= 75:
                # tenemos una cosa, haz que n sea el byte actual
                n = byte_actual
                # añade los siguientes n bytes como una cosa
                cosas.append(s.read(n))
                # aumenta el contador por n
                contador += n
            else:
    ...
    def serializar(self):
        ...
        for cosa in self.cosas:
            ...
            if type(cosa) == int:
                # convierte la cosa en un solo byte entero usando int_a_little_endian
                res += int_a_little_endian(cosa, 1)
            else:
                # en otro caso, esto es un elemento
                # obtén la longitud en bytes
                longitud = len(cosa)
                # convierte la longitud en un solo byte entero usando int_a_little_endian
    ...
    def dire(self, testnet=False):
        ...
        # HACK: siemplemente cuenta cuántos elementos hay para determinar de qué tipo es
