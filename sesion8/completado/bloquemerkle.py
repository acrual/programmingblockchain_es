...
class ÁrbolMerkle:
    ...
    def __init__(self, total):
        self.total = total
        # computa la máxima profundidad math.ceil(math.log(self.total, 2))
        self.profundidad_máxima = math.ceil(math.log(self.total, 2))
        # inicializa el atributo nodos para mantener el árbol real
        self.nodes = []
        ...
        for profundidad in range(self.profundidad_máxima + 1):
            ...
            # crea una lista de hashes de este nivel con el número correcto de cosas
            nivel_hashes = [None] * num_cosas
            # agrega los hashes de este nivel al árbol de merkle
            self.nodes.append(nivel_hashes)
        # pon el pointer hacia la raíz (profundidad=0, índice=0)
    ...
    def subir(self):
        # reduce profundidad en 1 y divide el índice
        self.profundidad_actual -= 1
        self.índice_actual //= 2

    def izquierda(self):
        # aumenta la profundidad e 1 y dobla el índice
        self.profundidad_actual += 1
        self.índice_actual *= 2

    def derecha(self):
        # aumenta la profundidad en 1 y doble el índice en 1
        self.profundidad_actual += 1
    ...
    def establece_nodo_actual(self, value):
        self.nodes[self.profundidad_actual][self.índice_actual] = value
    ...
    def poblar_árbol(self, flag_bits, hashes):
        ...
        while self.raíz() is None:
            # si somos una hoja, sabemos el hash de esta posición
            if self.es_hoja():
                # obtener el siguiente bit desde flag_bits: flag_bits.pop(0)
                flag_bits.pop(0)
                # establece el nodo actual en el árbol de merkle al siguiente hash: hashes.pop(0)
            ...
            # else
                ...
                # Ejercicio 6.2: obtener el hash derecho
                ...
                if hash_izquierdo is None:
                    ...
                    if flag_bits.pop(0) == 0:
                        # haz que el nodo actual sea el siguiente hash
                        self.establece_nodo_actual(hashes.pop(0))
                ...
                # Ejercicio 6.2: else
                    ...
                    # hemos completado este sub-árbol, subimos
                # Ejercicio 7.2: si el hash derecho existe
                    ...
                    # else
                        ...
                        # combina los hashes derecho e izquierdo
                ...
                # else
                    ...
                    self.subir()
        if len(hashes) != 0:
            raise RuntimeError('no todos los hashes han sido consumidos {}'.format(len(hashes)))
...
class BloqueMerkle:
    ...
    def es_válido(self):
        '''Verifica si la información del árbol merkle valida a la raíz de merkle'''
        # convierte el campo flags a un campo bit usando campo_bit_a_bytes
        flag_bits = campo_bit_a_bytes(self.flags)
        # dale la vuelta a los hashes para obtener nuestra lista de hashes para el cálculo de la raíz merkle
        hashes = [h[::-1] for h in self.hashes]
        # inicializa el árbol merkle
        árbol_merkle = ÁrbolMerkle(self.total)
        # puebla el árbol con flag bits y hashes
        árbol_merkle.poblar_árbol(flag_bits, hashes)
        # comprueba si la raíz computada [::-1] es la misma que la raíz merkle
        return árbol_merkle.raíz()[::-1] == self.raíz_merkle
