...
class NetworkEnvelope:
    ...
    def parsear(cls, s, testnet=False):
        ...
        if magia != magia_esperada:
            raise RuntimeError('la magia no está ok {} vs {}'.format(magia.hex(), magia_esperada.hex()))
        # comando 12 bytes
        comando = s.read(12)
        # elimina los 0's que arrastra
        comando = comando.strip(b'\x00')
        # la longitud de la carga es 4 bytes, little endian
        longitud_carga = little_endian_a_int(s.read(4))
        # checksum 4 bytes, primeros cuatro de doble-sha256 de carga
        checksum = s.read(4)
        ...
        if checksum_calculado != checksum:
            raise RuntimeError('checksum no encaja')
    ...
    def serializar(self):
        ...
        # la longitud de carga es 4 bytes, little endian
...
class MensajeVersión:
    ...
    def serializar(self):
        ...
        # servicios emisor es 8 bytes little endian
        ...
        res += self.nonce
        # agente_usuario es un campo variable, así que varint primero
        ...
        # retransmisión es 00 si es false, 01 si es verdadero
...
class ObtenerMensajeCabeceras:
    ...
    def serializar(self):
        ...
        # el bloque final está también in little-endian
...
class MensajeCabeceras:
    ...
    def parsear(cls, stream):
        ...
        bloques = []
        # haz un bucle tantas veces como el número de cabeceras
        for _ in range(num_cabeceras):
            # añade un bloque al array de bloques parseando el stream
            bloques.append(Bloque.parsear(stream))
            # lee el siguiente varint (num_txs)
            num_txs = read_varint(stream)
            # num_txs debería ser 0 o dar un RuntimeError
            if num_txs != 0:
                raise RuntimeError('el número txs no es 0')
...
class ObtenerMensajeDatos:
    ...
    def serializar(self):
        # empieza con el número de cosas como un varint
...
class NodoSimple:
    ...
    def handshake(self):
        '''Haz un handshake con el otro nodo. Handshake es enviar un mensaje de versión y recibir un verack de vuelta'''
        # crear un mensaje de versión
        versión = MensajeVersión()
        # enviar el comando
        self.enviar(versión.comando, versión.serializar())
        # esperar por el mensaje verack
        self.esperar_comandos({b'verack'})

    def enviar(self, comando, carga):
    ...
    def esperar_comandos(self, comandos):
        '''Espera por uno de los comandos en la lista'''
        ...
        while comando not in comandos:
            ...
            if comando == b'version':
                # enviar verack
                self.enviar(b'verack', b'')
            elif comando == b'ping':
                # enviar pong
                self.enviar(b'pong', envelope.carga)
        # devuelve el últio envelope que tenemos
        return envelope
