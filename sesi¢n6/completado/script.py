...
class Script:
    ...
    def parsear(cls, s):
        ...
        while contador < longitud:
            ...
            contador += 1
            # convierte el byte acutal en un entero
            byte_actual = actual[0]
            # si el byte actual está entre 1 and 75 incluidos
            if byte_actual >= 1 and byte_actual <= 75:
                # tenemos una cosa como n igual al byte actual
                n = byte_actual
                # añade los siguientes n bytes como una cosa
                cosas.append(s.read(n))
                # incrementa el contador por n
                contador += n
            else:
    ...
    def serializar(self):
        ...
        for cosa in self.cosas:
            ...
            if type(cosa) == int:
                ...
                # añade a res la longitud y la cosa
                res += prefijo + cosa
        # obtén la longitud de todo
        total = len(res)
        # codificar_varint al total longitud y anteponlo
    ...
    def sec_pubkey(self):
        '''devuelve el elmento pubkey asumiendo script sig p2pkh'''
        return self.cosas[1]

    def dire(self, testnet=False):
        '''Devuelve la dire correspondiente al script'''
        # HACK: cuenta cuántos elementos hay para determinar de qué tipo es
        if len(self.cosas) == 5:  # p2pkh
            # hash160 es el tercer elemento
            h160 = self.cosas[2]
            # convierte a dir ep2pkh usando h160_a_dire_p2pkh (recuerda testnet)
