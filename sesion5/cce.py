...
class ElementoCampo:
    ...
    def __add__(self, otro):
        if self.primo != otro.primo:
            raise TypeError('No puedes sumar dos números en Campos diferentes')
        # self.num y otro.num son los valores reales
        num = (self.num + otro.num) % self.primo
        # self.primo es contra lo que tendrás que hacer el módulo
        primo = self.primo
    ...
    def __pow__(self, n):
        ...
        # self.num**(p-1) % p == 1
        # deberás usar el operador % sobre n
        primo = self.primo
...
class Punto:
    ...
    def __init__(self, x, y, a, b):
        ...
        self.x = x
        self.y = y
        # x es None e y None representa el punto en el infinito
        # Compruébalo aquí ya que sino la ecuación de abajo no tendrá sentido
...
class PruebaPunto(TestCase):
    ...
    def prueba_en_curva(self):
        ...
        # estos no deberían dar un error
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        # prueba si los siguientes puntos están o no en la curva
        # y^2=x^3-7 sobre F_223:
        # (192,105) (17,56) (200,119) (1,193) (42,99)
        # los que no estén deberían dar un ValueError
        primo = 223
        ...
        for x_bruto, y_bruto in puntos_inválidos:
            ...
            x = ElementoCampo(x_bruto, primo)
            y = ElementoCampo(y_bruto, primo)
            # comprueba que crear el punto resulte en un ValueError
            # with self.assertRaises(ValueError):
            #     Punto(x, y, a, b)
            with self.assertRaises(ValueError):
                Punto(x, y, a, b)
    ...
    def prueba_sumar(self):
        # comprueba las siguientes sumas sobre la curva y^2=x^3-7 sobre F_223:
        # (192,105) + (17,56)
        # (47,71) + (117,141)
        # (143,98) + (76,66)
        primo = 223
    ...
    def prueba_rmul(self):
        # comrpueba las siguientes multiplicaciones escalares
        # 2*(192,105)
        # 2*(143,98)
        # 2*(47,71)
        # 4*(47,71)
        ...
        for s, x1_bruto, y1_bruto, x2_bruto, y2_bruto in multiplicaciones:
            ...
            if x2_bruto is None:
                ...
                y2 = ElementoCampo(y2_bruto, primo)
                p2 = Punto(x2, y2, a, b)
            # comrpueba que el producto sea igual al punto esperado
...
class PuntoS256(Punto):
    ...
    def __rmul__(self, coeficiente):
        # haremos el módulo por N para hacer esto simple
        coef = coeficiente % N
        return super().__rmul__(coef)

    def sec(self, comprimido=True):
        # devuelve la versión binaria del formato sec, NO hex
        # si está comprimido, empieza por b'\x02' si self.y.num es par, b'\x03' si self.y es impar
        # luego self.x.num
        ...
        if comprimido:
            ...
            # si no está comprimido, empieza con b'\x04' seguido por self.x y luego self.y
    ...
    def dire(self, comprimido=True, testnet=False):
        ...
        # hash160 the sec
        h160 = hash160(sec)
        # bruto es el hash 160 anteopuesto por b'\x00' para mainnet, b'\x6f' para testnet
        if testnet:
            prefijo = b'\x6f'
        else:
            prefijo = b'\x00'
        # devuelve el codifica_base58_checksum of the prefix and h160
        return codificar_base58_checksum(prefijo + h160)

    def verificar(self, z, firma):
        # recuerda firma.r y firma.s son las principales cosas que estamos comprobando
    ...
    def parsear(self, sec_bin):
        '''returns a Punto object from a comprimido sec binary (not hex)
        '''
        if sec_bin[0] == 4:
...
class S256Test(TestCase):
    ...
    def test_pubPunto(self):
        # write a test that tests the public Punto for the following
        Puntos = (
            # secret, x, y
        ...
        # iterate over Puntos
        for secret, x, y in Puntos:
            # initialize the secp256k1 Punto (PuntoS256)
            Punto = PuntoS256(x, y)
            # check that the secret*G is the same as the Punto
            self.assertEqual(secret * G, Punto)

    def test_sec(self):
        coeficiente = 999**3
        uncomprimido = '049d5ca49670cbe4c3bfa84c96a8c87df086c6ea6a24ba6b809c9de234496808d56fa15cc7f3d38cda98dee2419f415b7513dde1301f8643cd9245aea7f3f911f9'
        comprimido = '039d5ca49670cbe4c3bfa84c96a8c87df086c6ea6a24ba6b809c9de234496808d5'
        Punto = coeficiente * G
        self.assertEqual(Punto.sec(comprimido=False), bytes.fromhex(uncomprimido))
        self.assertEqual(Punto.sec(comprimido=True), bytes.fromhex(comprimido))
        coeficiente = 123
        uncomprimido = '04a598a8030da6d86c6bc7f2f5144ea549d28211ea58faa70ebf4c1e665c1fe9b5204b5d6f84822c307e4b4a7140737aec23fc63b65b35f86a10026dbd2d864e6b'
        comprimido = '03a598a8030da6d86c6bc7f2f5144ea549d28211ea58faa70ebf4c1e665c1fe9b5'
        Punto = coeficiente * G
        self.assertEqual(Punto.sec(comprimido=False), bytes.fromhex(uncomprimido))
        self.assertEqual(Punto.sec(comprimido=True), bytes.fromhex(comprimido))
        coeficiente = 42424242
        uncomprimido = '04aee2e7d843f7430097859e2bc603abcc3274ff8169c1a469fee0f20614066f8e21ec53f40efac47ac1c5211b2123527e0e9b57ede790c4da1e72c91fb7da54a3'
    ...
    def test_dire(self):
        secret = 888**3
        mainnet_dire = '148dY81A9BmdpMhvYEVznrM45kWN32vSCN'
        testnet_dire = 'mieaqB68xDCtbUBYFoUNcmZNwk74xcBfTP'
        Punto = secret * G
        self.assertEqual(
            Punto.dire(comprimido=True, testnet=False), mainnet_dire)
        self.assertEqual(
            Punto.dire(comprimido=True, testnet=True), testnet_dire)
        secret = 321
        mainnet_dire = '1S6g2xBJSED7Qr9CYZib5f4PYVhHZiVfj'
        testnet_dire = 'mfx3y63A7TfTtXKkv7Y6QzsPFY6QCBCXiP'
        Punto = secret * G
        self.assertEqual(
            Punto.dire(comprimido=False, testnet=False), mainnet_dire)
        self.assertEqual(
            Punto.dire(comprimido=False, testnet=True), testnet_dire)
        secret = 4242424242
        mainnet_dire = '1226JSptcStqn4Yq9aAmNXdwdc2ixuH9nb'
        testnet_dire = 'mgY3bVusRUL6ZB2Ss999CSrGVbdRwVpM8s'
        Punto = secret * G
        self.assertEqual(
            Punto.dire(comprimido=False, testnet=False), mainnet_dire)
        self.assertEqual(
...
class ClavePrivada:
    ...
    def firmar(self, z):
        ...
        if s > N / 2:
            s = N - s
        # devuelve una instancia de Firma:
        # Firma(r, s)
        return Firma(r, s)
