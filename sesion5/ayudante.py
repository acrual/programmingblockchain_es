...
def int_a_little_endian(n, long):
    '''int_a_little_endian toma un entero y devuelve la secuencia little-endian
    de bytes de longitud long'''
...
def codificar_base58(s):
    ...
    # convierte desde binario a hex, y luego hex a entero
    num = int(s.hex(), 16)
    res = bytearray()
    while num > 0:
...
def codificar_base58_checksum(bruto):
    '''Toma bytes y los convierte en codificación en base58 con checksum'''
    # checksum son los primeros 4 bytes of the double_sha256
    checksum = doble_sha256(bruto)[:4]
    # codificar_base58 bruto y el checksum
    base58 = codificar_base58(bruto + checksum)
    # convierte en cadena con base58.decode('ascii')
...
def h160_a_dire_p2pkh(h160, testnet=False):
    ...
    # devuelve el codificar_base58_checksum de prefijo y h160
