...
def codificar_base58_checksum(bruto):
    '''Toma bytes y los convierte en una codificación base58 con checksum'''
...
def read_varint(s):
    ...
    if i == 0xfd:
        ...
        # cualquier otra cosa es simplemente el entero
        return i


def codificar_varint(i):
    '''codificar un entero como varint'''
    if i < 0xfd:
        return bytes([i])
    elif i < 0x10000:
...
def h160_a_dire_p2pkh(h160, testnet=False):
    ...
    if testnet:
        ...
        prefijo = b'\x00'
    # devuelve codificar_base58_checksum de prefijo y h160
...
def h160_a_dire_p2sh(h160, testnet=False):
    ...
    # p2sh tiene un prefjio de b'\x05' para mainnet, b'\xc4' para testnet
    if testnet:
        prefijo = b'\xc4'
    else:
        prefijo = b'\x05'
