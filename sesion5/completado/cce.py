...
class Punto:
    ...
    def __add__(self, otro):
        ...
        # Case 0.0: self es el punto en el infinito, devuelve otro
        if self.x is None:
            return otro
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        ...
        # los que no estén deberían dar un error ValueError
        primo = 223
...
class PuntoS256(Punto):
    ...
    def __rmul__(self, coeficiente):
        # haremos módulo por N para hacer esto simple
        coef = coeficiente % N
    ...
    def dire(self, comprimido=True, testnet=False):
        ...
        # hash160 el sec
        h160 = hash160(sec)
        # bruto es el hash 160 antepuesto por b'\x00' par mainnet, b'\x6f' para testnet
        if testnet:
            prefijo = b'\x6f'
        else:
            prefijo = b'\x00'
        # devuelve el codificar_base58_checksum del prefijo y h160
    ...
    def parsear(self, sec_bin):
        ...
        # resuelve por la parte izquierda
        beta = alpha.sqrt()
        if beta.num % 2 == 0:
            beta_par = beta
...
class PruebaClavePrivada(TestCase):
    ...
    def prueba_firmar(self):
        ...
        z = randint(0, 2**256)
        firmar = pk.firmar(z)
        self.assertTrue(pk.punto.verificar(z, firmar))
