...
def little_endian_a_int(b):
    '''little_endian_a_int toma una secuencia en bytes como un número little-endian.
    Devuelve un entero'''
...
def encode_base58(s):
    ...
    # convertir desde binario a hex y entonces desde hex a entero
    num = int(s.hex(), 16)
    res = bytearray()
    while num > 0:
...
def read_varint(s):
    ...
    i = s.read(1)[0]
    if i == 0xfd:
        # 0xfd significa que los dos próximos bytes son el número
        return little_endian_a_int(s.read(2))
    elif i == 0xfe:
        # 0xfe significa que los cuatro próximos bytes son el número
