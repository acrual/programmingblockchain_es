...
class Script:
    ...
    def tipo(self):
        ...
        if len(self.elementos) == 0:
           ...
           and self.elementos[-1] == 0x87:
            ...
            # HACK: asume que p2sh es un multi-firma
            # p2sh multifirma:
    ...
    def sec_pubkey(self, index=0):
        '''index no se usa para p2pkh, para p2sh, significa una de m claves públicas'''
        tipo_firma = self.tipo()
        if tipo_firma == 'p2pkh sig':
            return self.elementos[1]
        elif tipo_firma == 'p2sh sig':
            # HACK: asume que p2sh es una multifirma (multisig)
