...
class Tx:
    ...
    def comision(self):
        ...
        # inicializa la suma de inputs y de outputs
        input_sum, output_sum = 0, 0
        # iterate sobre los inputs
        for tx_in in self.tx_ins:
            # para cada input obtén el valor y añádelo al input sum
            input_sum += tx_in.valor()
        # itera sobre los outputs
        for tx_out in self.tx_outs:
            # para cada output obtén la cantidad y añádelo a la suma de outputs
            output_sum += tx_out.cantidad
        # devuelve input sum - output sum
        return input_sum - output_sum

    def sig_hash(self, indice_input, tipo_hash):
        '''Devuelve la representación entera del hash que necesita ser
        firmado para index input_index'''
        # crea un nuevo grupo de tx_ins (alt_tx_ins)
        alt_tx_ins = []
        ...
        # añáde el input al input_index
        input_firmante = alt_tx_ins[indice_input]
        # obtén el script script_pubkey del input
        script_pubkey = input_firmante.script_pubkey(self.testnet)
        # el script_sig del signing_input debería ser script_pubkey
...
class TxIn:
    ...
    def obtener_tx(self, testnet=False):
        if self.tx_previa not in self.cache:
            ...
            try:
                ...
                raise RuntimeError('Obtenido desde el servidor: {}'.format(response))
    ...
    def valor(self, testnet=False):
        '''Obtén el valor outpoint value mirando el hash de la tx en el servidor libbitcoin
    ...
    def script_pubkey(self, testnet=False):
        '''Obtén el scriptPubKey buscando el hash de la tx en el servidor libbitcoin
        Devuelve el scriptpubkey binario
        '''
        # usa self.obtener_tx para obtener la transacción
        tx = self.obtener_tx(testnet=testnet)
        # obtén el output en self.indice_previo
        # devuelve el atributo script_pubkey y serializa
        return tx.tx_outs[self.indice_previo].script_pubkey

    def firma_der(self, index=0):
        '''devuelve un firma en formato DER y tipo_hash si el script_sig
        tiene una firma'''
...
class PruebaTx(TestCase):
    ...
    def prueba_comisión(self):
        ...
        stream = BytesIO(tx_bruta)
        tx = Tx.parsear(stream)
        self.assertEqual(tx.comision(), 40000)
        ...
        stream = BytesIO(tx_bruta)
        tx = Tx.parsear(stream)
        self.assertEqual(tx.comision(), 140500)
