...
class ElementoCampo:
    ...
    def __init__(self, num, primo):
        ...
        if self.num >= self.primo or self.num < 0:
            error = 'El número {} no está en el rango del campo de 0 a {}'.format(
    ...
    def __truediv__(self, otro):
        ...
        # self.prime es contra lo que deberás calcular el modulo
        primo = self.primo
...
class Punto:
    ...
    def __init__(self, x, y, a, b):
        ...
        self.x = x
        self.y = y
        # x e y valiendo None, representa el punto en el infinito
        # Compruébalo pues de otra forma la ecuación no tendrá sentido
        # con ambos valores igual a None
        if self.x is None and self.y is None:
            return
        # asegúrate de que se satisface la ecuación de la curva
        # y**2 == x**3 + a*x + b
        if self.y**2 != self.x**3 + a*x + b:
        # sino, lanza un RuntimeError
    ...
    def __rmul__(self, coeficiente):
        ...
        # impleméntalo de manera ingenua:
        # comienza producto desde cero (punto en el infinito)
        ...
        for _ in range(coeficiente):
            # continua añadiendo self una y otra vez
            producto += self
        ...
        # sigue doblando el punto y si el bit está ahí para el coeficiente
        # añade el actual
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        # comprueba si los siguientes puntos están o no en la curva
        # y^2=x^3-7 sobre F_223:
        # (192,105) (17,56) (200,119) (1,193) (42,99)
        # los que no estén deberían dar un error RuntimeError
        primo = 223
        a = ElementoCampo(0, primo)
        b = ElementoCampo(7, primo)
        
        puntos_validos = ((192,105), (17,56), (1,193))
        puntos_invalidos = ((200,119), (42,99))
        
        # itera sobrelos puntos válidos
        for x_bruto, y_bruto in puntos_validos:
        ...
        for x_bruto, y_bruto in puntos_invalidos:
    ...
    def prueba_suma1(self):
        ...
        # itera sobre las siguientes sumas
...
class PuntoS256(Punto):
    ...
    def __rmul__(self, coeficiente):
        # haremos módulo por N para hacer esto sencillo
        coef = coeficiente % N
        # actual pasará por una expansión binaria
        actual = self
        # res es lo que devolveremos, empezaremos por 0
        res = PuntoS256(None, None)
        # doblamos 256 veces y añadimos donde haya un 1 en la
        # representación binaria del coeficiente
    ...
    def verificar(self, z, fir):
        # recuerda que fir.r y fir.s son las principales cosas que estamos comprobando
        # recuerda que 1/s = pow(s, N-2, N)
        s_inv = pow(fir.s, N-2, N)
        # u = z / s
        u = z * s_inv % N
        # v = r / s
        v = fir.r * s_inv % N
        # u*G + v*P debería tener r como coordenada-x
        total = u*G + v*self
        return total.x.num == fir.r
...
class PruebaS256(TestCase):
    ...
    def prueba_puntopub(self):
        ...
        for secreto, x, y in puntos:
            # iniciliaza el punto secp256k1 (PuntoS256)
            punto = PuntoS256(x, y)
...
class ClavePrivada:
    ...
    def firmar(self, z):
        ...
        k = randint(0, 2**256)
        # r es la coordenada-x del punto k*G resultante
        r = (k*G).x.num
    ...
    def wif(self, comprimido=True, testnet=False):
        # convierte el secreto de entero a big-endian 32-bytes usando num.to_bytes(32, 'big')
        secreto_bytes = self.secreto.to_bytes(32, 'big')
        ...
        return encode_base58_checksum(prefijo + secreto_bytes + sufijo)
...
class PruebaClavePrivada(TestCase):
    ...
    def prueba_wif(self):
        ...
        esperada = 'L5oLkpV3aqBJ4BgssVAsax1iRa77G5CVYnv9adQ6Z87te7TyUdSC'
        self.assertEqual(pk.wif(comprimido=True, testnet=False), esperada)
        pk = ClavePrivada(2**256-2**201)
        esperada = '93XfLeifX7Jx7n7ELGMAf1SUR6f9kgQs8Xke8WStMwUtrDucMzn'
        self.assertEqual(pk.wif(comprimido=False, testnet=True), esperada)
        pk = ClavePrivada(0x0dba685b4511dbd3d368e5c4358a1277de9486447af7b3604a69b8d9d8b7889d)
        esperada = '5HvLFPDVgFZRK9cd4C5jcWki5Skz6fmKqi1GQJf5ZoMofid2Dty'
        self.assertEqual(pk.wif(comprimido=False, testnet=False), esperada)
        pk = ClavePrivada(0x1cca23de92fd1862fb5b76e5f4f50eb082165e5191e116c18ed1a6b24be6a53f)
        esperada = 'cNYfWuhDpbNM1JWc3c6JTrtrFVxU4AGhUKgw5f93NP2QaBqmxKkg'
        self.assertEqual(pk.wif(comprimido=True, testnet=True), esperada)
