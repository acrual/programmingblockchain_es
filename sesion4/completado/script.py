...
class Script:
    ...
    def tipo(self):
        '''Algunos scripts pay-to estándares'''
        if len(self.elementos) == 0:
           ...
           and self.elementos[-1] == 0x87:
            ...
            # HACK: asume que p2sh es un multisig
            # p2sh multisig:
    ...
    def firma(self, index=0):
        '''index no se usa para p2pkh, para p2sh, significa 1 de m firmas'''
        tipo_firma = self.tipo()
        if tipo_firma == 'p2pkh sig':
            return self.elementos[0]
        elif tipo_firma == 'p2sh sig':
            return self.elementos[index+1]
        else:
            raise RuntimeError('el tipo de script necesita ser p2pkh sig o p2sh sig')

    def sec_pubkey(self, index=0):
        '''index no se usa para p2pkh, para p2sh, significa 1 de n claves públicas'''
        firma_tipo = self.tipo()
        if firma_tipo == 'p2pkh sig':
            return self.elementos[1]
        elif firma_tipo == 'p2sh sig':
...
class PruebaScript(TestCase):
    ...
    def prueba_p2pkh(self):
        ...
        self.assertEqual(script_sig.tipo(), 'p2pkh sig')
