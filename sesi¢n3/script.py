...
class Script:
    ...
    def tipo(self):
        '''Algunos scripts standard del tipo pay-to'''
    ...
    def firma(self, index=0):
        ...
        fir_type = self.tipo()
        if fir_type == 'p2pkh sig':
            return self.elementos[0]
        elif fir_type == 'p2sh sig':
            return self.elementos[index+1]
        else:
            raise RuntimeError('el tipo de script debe p2pkh sig o p2sh sig')

    def sec_pubkey(self, index=0):
        '''index no se usa para p2pkh, para p2sh, significa 1 de n pubkeys'''
        fir_type = self.tipo()
        if fir_type == 'p2pkh sig':
