...
class ElementoCampo:
    ...
    def __init__(self, num, primo):
        ...
        if self.num >= self.primo or self.num < 0:
            error = 'El número {} no está en el rango del campo 0 a {}'.format(
...
class Punto:
    ...
    def __init__(self, x, y, a, b):
        ...
        self.x = x
        self.y = y
        # si x es None e y es None, entonces es el punto en el infinito
        # Comprueba si es así porque en otro caso la ecuación de abajo no tendrá sentido
        # si ambos tienen un valor de None.
        if self.x is None and self.y is None:
            return
        # asegúrate de que la curva elíptica se cumple
        # y**2 == x**3 + a*x + b
    ...
    def __add__(self, otro):
        if self.a != otro.a or self.b != otro.b:
            raise RuntimeError('Puntos {}, {} no están en la misma curva'.format(self, otro))
    ...
    def __rmul__(self, coeficiente):
        ...
        # impleméntalo de forma inocente:
        # comienza el producto desde 0 (punto en el infinito)
        # usa: self.__class__(None, None, a, b)
        producto = self.__class__(None, None, self.a, self.b)
        # haz un bucle coeficiente veces
        # use: for _ in range(coeficiente):
        ...
        return producto
        # Extra:
        # una técnica más avanzada consiste en doblar el punto
...
class PruebaPunto(TestCase):
    ...
    def prueba_en_curva(self):
        ...
        # estas no deberían levantar un error
...
class PruebaCCE(TestCase):
    ...
    def prueba_en_curva(self):
        # prueba si los siguientes puntos están en la curva o no
        # y^2=x^3-7 sobre F_223:
        # (192,105) (17,56) (200,119) (1,193) (42,99)
        # los que no estén deberían levantar un RuntimeError
        primo = 223
        ...
        for x_bruto, y_bruto in puntos_inválidos:
            ...
            x = ElementoCampo(x_bruto, primo)
            y = ElementoCampo(y_bruto, primo)
            # comprueba si crear los puntos resulta en un RuntimeError
            # with self.assertRaises(RuntimeError):
            #     Punto(x, y, a, b)
            with self.assertRaises(RuntimeError):
                Punto(x, y, a, b)
    ...
    def prueba_rmul(self):
        ...
        for s, x1_bruto, y1_bruto, x2_bruto, y2_bruto in multiplicaciones:
            ...
            y1 = ElementoCampo(y1_bruto, primo)
            p1 = Punto(x1, y1, a, b)
            # inicializa el segundo punto según sea el punto en el infinito
            # x2 = ElementoCampo(x2_bruto, primo)
            # y2 = ElementoCampo(y2_bruto, primo)
            # p2 = Punto(x2, y2, a, b)
            if x2_bruto is None:
...
class PuntoS256(Punto):
    ...
    def __rmul__(self, coeficiente):
        # haremos el mod por N para hacer esto sencillo
        coef = coeficiente % N
        # llevaremos a cabo una expansión binaria en actual
        actual = self
        # res es lo que devolvemos, comienza en 0
        res = PuntoS256(None, None)
        # doblamos 256 veces y añadimos aquel donde haya 1 
        # en la representación binaria del coeficiente
    ...
    def sec(self, comprimido=True):
        ...
        # si no está comprimido, empieza con b'\x04' seguido de self.x y luego de self.y
    ...
    def dire(self, comprimido=True, testnet=False):
        ...
        # haz hash160 de sec
        h160 = hash160(sec)
        # bruto es hash160 antepuesto por b'\x00' para mainnet, b'\x6f' para testnet
        if testnet:
            prefijo = b'\x6f'
        else:
            prefijo = b'\x00'
        ...
        # haz encode_base58 de bruto + checksum
        dire = encode_base58(bruto+checksum)
        # devuelve como cadena, puedes usar .decode('ascii')
    ...
    def verificar(self, z, firma):
        ...
        # v = r / s
        v = firma.r * s_inv % N
        # u*G + v*P debería tener como coordenada-x la r
        total = u*G + v*self
        return total.x.num == firma.r

    @classmethod
    def parsear(self, sec_bin):
        '''devuelve un objeto Punto como sec binario comprimido (no hex)
        '''
        if sec_bin[0] == 4:
...
class PruebaS256(TestCase):
    ...
    def prueba_puntopub(self):
        # escribe una prueba del punto público para los siguientes puntos:
        puntos = (
            # secreto, x, y
        ...
        for secreto, x, y in puntos:
            # inicializa tel punto secp256k1 (PuntoS256)
            punto = PuntoS256(x, y)
            # comprueba que secret*G sea igual al punto
...
class Firma:
    ...
    def der(self):
        ...
        # si sbin tiene un bit alto, añade 00
        if sbin[0] >= 128:
            sbin = b'\x00' + sbin
...
class ClavePrivada:
    ...
    def firmar(self, z):
        ...
        k = randint(0, 2**256)
        # r es la coordenada-xx del punto resultante k*G
        r = (k*G).x.num
        ...
        if s > N/2:
            s = N - s
        # Devolver una instancia de la firma:
        # Firma(r, s)
        return Firma(r, s)

    def wif(self, comprimido=True, testnet=False):
        # convertir el secreto de entero a big-endian 32-bytes usando num.to_bytes(32, 'big')
